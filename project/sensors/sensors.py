...
def publish_battery(max_battery, battery_value, battery_consumption, non_battery_consumption, timestamp):
...
# Print the members in a tabular format
...
def update_activation_status():
    ...
    print(f"Activation request received for {member_id}, {consumer_id} ", flush=True)
    
    if member_id in members and consumer_id in members[member_id]["consumers"]:
        
        members[member_id]["consumers"][consumer_id]["activated"] = True
        response.content_type = 'application/json'
        return json.dumps({"status": "success"})
    else:
        response.status = 400
...
if __name__ == '__main__':
    ...
    while True:
        ...
        print_members_in_table(members)
        ...
        battery_value = max(min(battery_value + total_production, battery_info["max-capacity"]), 0)
        if total_consumption <= battery_value:
            battery_value -= total_consumption
            battery_consumption = total_consumption
            non_battery_consumption = 0
        else:
            battery_consumption = battery_value
            non_battery_consumption = total_consumption - battery_value
            battery_value = 0
        
        
        # Publish the battery delta
        publish_battery(battery_info["max-capacity"], battery_value, battery_consumption, non_battery_consumption, timestamp)
